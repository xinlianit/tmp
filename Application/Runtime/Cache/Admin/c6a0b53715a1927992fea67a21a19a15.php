	...
	<ul class="breadcrumb">
                                    ...
                                    <div class="table-search">
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-2 seach-input-col">
                                                <input type="text" id="search-key" name="search-key" placeholder="商户名称" value="<?php echo ($search_key); ?>" class="input-sm seach-input">
                                            </div>
                                            <span class="input-group-btn search-btn">
                                                <button type="button" class="btn btn-purple btn-sm" id="search-button" surl='<?php echo U("Adv/position");?>'>
                                                    查询
		...
		<td colspan="100" style="text-align:center;">暂无数据！</td>
                                        ...
                                        <?php if(is_array($page["list"])): $i = 0; $__LIST__ = $page["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>                                                
                                                <td colspan="8"><span class="lbl"></span> <a style="cursor: pointer" hotel_id='<?php echo ($list["id"]); ?>' class="spread icon-plus-sign" herf='javascript:void(0)'></a> <?php echo ($list["hotel_name"]); ?></td>
                                            </tr>
                                            <?php if(is_array($list["list"])): $i = 0; $__LIST__ = $list["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="tr_hotel_<?php echo ($vo["hotel_id"]); ?> hide">                                                
                                                <td>&nbsp;</td>
                                                <td><?php echo ($vo["advert_id"]); ?></td>
                                                <td><?php echo ($vo["advert_info"]["advert_name"]); ?></td>
                                                <td><?php switch($vo["release_source"]): case "1": ?>平台发布<?php break;?>
                                                        <?php default: ?>酒店发布<?php endswitch;?></td>
                                                <td><?php echo ($vo["advert_info"]["adver_name"]); ?></td>
                                                <td>首页广告</td>
                                                <td><?php echo (date('Y-m-d',$vo["begin_time"])); ?> - <?php echo (date('Y-m-d',$vo["end_time"])); ?></td>
                                                <td>
                                                    <button type="button" class="btn btn-up" r_id='<?php echo ($vo["id"]); ?>' h_id='<?php echo ($vo["hotel_id"]); ?>' url='<?php echo U("Adv/changeSort");?>'>上移</button>
                                                    <button type="button" class="btn btn-down" r_id='<?php echo ($vo["id"]); ?>' h_id='<?php echo ($vo["hotel_id"]); ?>' url='<?php echo U("Adv/changeSort");?>'>下移</button>
                                                </td>
