	...
	<form name="" action="" method="get">
		...
		<td colspan="100" style="text-align:center;">暂无数据！</td>
											...
											<?php if(is_array($page["list"])): $i = 0; $__LIST__ = $page["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
													...
													<td>
														<?php if(is_array($list["device"])): $i = 0; $__LIST__ = $list["device"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$dev): $mod = ($i % 2 );++$i;?><ul class="dev_detal">
																<li><?php echo ($dev["device_address"]); ?></li>
																<li><?php echo ($dev["position"]); ?></li>
																<li><?php echo ($dev["app_version"]); ?></li>
