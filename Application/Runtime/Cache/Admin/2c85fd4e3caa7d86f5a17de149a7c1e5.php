                                ...
                                <div class="table-responsive">
                                    <div class="table-search">
                                        <div class="form-group">
                                            <label class="col-sm-3 control-label no-padding-right search-label" for="form-field-1">地区名称：</label>
                                            ...
                                            <span class="input-group-btn search-btn">
                                                <button type="button" class="btn btn-purple btn-sm" id="mart-search-button" surl='<?php echo U("Baseinfo/mart");?>'>
                                                    查询
                                    ...
                                    <div class="table-search">
                                        <div class="form-group">
                                            <span class="input-group-btn search-btn">
                                                <button event-name="add-mart" type="button" class="btn btn-sm btn-success">
                                                    新增
                                                    <i class="icon-plus smaller-75"></i>
                                                </button>
                                                <div class="separate-2"><i class="separate-flag"></i></div>
                                                <button event-name="dels-mart" type="button" class="btn btn-sm btn-default">
                                                    删除
                                            ...
                                            <td colspan="6">
                                                ...
                                                <?php if(is_array($area)): $i = 0; $__LIST__ = $area;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><div class="tree_box_1">
                                                        <div class="tree_box_1_title" style='height:35px; line-height: 35px;margin: 5px 0'>
                                                            <input type="checkbox" class="ace" name="area_id" value="<?php echo ($list["id"]); ?>"  /><span class="lbl"></span> <a style="cursor: pointer" class="spread icon-plus-sign" herf='javascript:void(0)'></a> <?php echo ($list["city_name"]); ?>
                                                        </div>                                                        
                                                        <div class="tree_box_2 hide" tree_box_child="<?php echo ($list["id"]); ?>">
                                                            <table class="table table-striped table-bordered table-hover">
                                                                <?php if(is_array($list["child"])): $i = 0; $__LIST__ = $list["child"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list2): $mod = ($i % 2 );++$i;?><tr>
                                                                    <td  class="align-right" style="width:5%">
                                                                        <input type="checkbox" class="ace" name="area_id" value="<?php echo ($list2["id"]); ?>" pvalue="<?php echo ($list["id"]); ?>" /><span class="lbl"></span>
                                                                    </td>
                                                                    <td style="width:95%"> <a style="cursor: pointer" class="spread2 icon-plus-sign" herf='javascript:void(0)'></a> <?php echo ($list2["city_name"]); ?></td>
                                                                </tr> 
                                                                <tr  class="hide"  tree_box_child="<?php echo ($list2["id"]); ?>"> 
                                                                    <td  colspan="2">
                                                                        <table class="table table-striped table-bordered table-hover">
                                                                            <?php if(is_array($list2["child"])): $i = 0; $__LIST__ = $list2["child"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list3): $mod = ($i % 2 );++$i;?><tr>
                                                                                    <td  class="align-right" style="width:10%">
                                                                                        <input type="checkbox" class="ace" name="area_id" value="<?php echo ($list3["id"]); ?>" pvalue="<?php echo ($list2["id"]); ?>" /><span class="lbl"></span>
                                                                                    </td>
                                                                                    <td style="width:5%"><?php echo ($list3["city_name"]); ?></td>
                                                                                    <td style="width:70%">
                                                                                    <?php if(count($list3['mart']) > 0): if(is_array($list3["mart"])): foreach($list3["mart"] as $key=>$vo): echo ($vo["area_name"]); ?>;<?php endforeach; endif; ?>
                                                                                    <?php else: ?>
                                                                                    <p class="muted">暂无商圈</p><?php endif; ?>
                                                                                    </td>
                                                                                    <td class="hidden-480">
                                                                                        <div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
                                                                                            <a class="green" event-name="edit-mart" event-id="<?php echo ($list3["id"]); ?>" href="javascript:void(0);" title="编辑">
                                                                                                <i class="icon-pencil bigger-130"></i>
                                                                                            </a>

                                                                                            <a class="red" event-name="del-mart"  event-id="<?php echo ($list3["id"]); ?>" href="javascript:void(0);" title="删除">
...
<!-- 添加面板 -->
<div dialog-id="add-mart-box" class="hide">
    <form class="form-horizontal" role="form" form-id="add-mart-form" action="<?php echo U('Baseinfo/addMart');?>" method="post">
        <div class="form-group">
                <label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 商户地址：</label>
                <div class="col-sm-10">
                        <div id="city-component1" class="city-component" default-province="440000" default-city="440300" default-region="440304"></div>
                </div>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-10"><i class="spread icon-plus-sign"></i>&nbsp;<a  class="mart_add" href="javascript:void(0)">添加商圈</a></div>
        </div>
        <div class="space-4"></div>        
    </form>
</div>
<!-- 编辑面板 -->
<div dialog-id="edit-mart-box" class="hide">
    <form class="form-horizontal" role="form" form-id="edit-mart-form" action="<?php echo U('Baseinfo/editMart');?>" method="post">
        <div class="form-group">
                <label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 商户地址：</label>
                <div class="col-sm-10">
                        <div id="city-component2" class="city-component" default-province="440000" default-city="440300" default-region="440304"></div>
                </div>
        </div>
...
<!-- 删除面板 -->
<div dialog-id="del-mart-box" class="hide">
    <h3>确定要删除吗？</h3>
    <div>删除后不可恢复！</div>
    <form class="form-horizontal" role="form" form-id="del-mart-form" action="<?php echo U('Baseinfo/delMart');?>" method="post">
        <input type="hidden" name="cid" >
    </form>
</div>
<!-- 批量删除面板 -->
<div dialog-id="dels-mart-box" class="hide">
    <h3>确定要批量删除吗？</h3>
    <div>删除后不可恢复！</div>
    <form class="form-horizontal" role="form" form-id="dels-mart-form" action="<?php echo U('Baseinfo/delMart');?>" method="post">
