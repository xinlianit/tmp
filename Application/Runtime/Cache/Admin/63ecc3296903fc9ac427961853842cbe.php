	...
	<div class="main-container-inner">
							...
							<div class="col-xs-12">
								...
								<div class="table-responsive">
									<div class="table-search">
										<form name="" action="<?php echo U('Node/merchant');?>" method="get">
									...
									<div class="table-search">
										<div class="form-group">
											<span class="input-group-btn search-btn">
												...
												<button event-name="on-off" tag-id="on" table-name="list" url="<?php echo U('Node/busineAccOnOff');?>" message="" type="button" class="btn btn-sm btn-primary">
													启用
													<i class="icon-unlock smaller-75"></i>
												</button>
												<div class="separate"></div>
												<button event-name="on-off" tag-id="off" table-name="list" url="<?php echo U('Node/busineAccOnOff');?>" message="确认要停用选中的 x 条账号？|停用后账号将无法正常使用！" type="button" class="btn btn-sm btn-warning">
													停用
													<i class="icon-lock smaller-75"></i>
												</button>
												<div class="separate-2"><i class="separate-flag"></i></div>
												
												<button event-name="delete" table-name="list" url="<?php echo U('Node/deleteBusiAcc');?>" message="确认要删选中的  x 条商户账号？|删除后信息不可恢复！" type="button" class="btn btn-sm btn-default">
													批量删除
		...
		<td colspan="100" style="text-align:center;">暂无数据！</td>
											...
											<?php if(is_array($page["list"])): $i = 0; $__LIST__ = $page["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
													...
													<td><?php echo (_default(get_field_status("STAFF_INFO","ACCOUNT_TYPE",$list["account_type"],"NAME"))); ?></td>
													<td><?php echo (_default($list["staff_account"])); ?></td>
													<td><?php echo (_default($list["staff_name"])); ?></td>
													<td><?php echo (_default(get_field_status("STAFF_INFO","DATA_STATUS",$list["data_status"],"NAME"))); ?></td>
													<td>
														<div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
															<a class="green" event-name="addEditAcc" url="<?php echo U('Node/busineAccInfo');?>" params="id=<?php echo ($list["id"]); ?>"  dialog-title="商户账号编辑" submit-title="保存" flag="edit" href="javascript:void(0);" title="编辑">
																<i class="icon-pencil bigger-130"></i>
															</a>
	
															<a class="red" event-name="delete" id-value="<?php echo ($list["id"]); ?>" url="<?php echo U('Node/deleteBusiAcc');?>" message="确认要删除此商户账号？|删除后信息不可恢复！" href="javascript:void(0);" title="删除">
...
<div dialog-id="addEdit" class="hide">
	...
	<form class="form-horizontal" role="form" form-id="addEdit" action="<?php echo U('Node/addEditBusiAcc');?>" method="post">
			...
			<div class="form-group">
				...
				<div class="col-sm-4" style="width:57.3333%;">
					<select tag-id="data-sort" name="account_type" class="form-control">
						...
						<?php $_result=get_field_status('STAFF_INFO','ACCOUNT_TYPE');if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$type_list): $mod = ($i % 2 );++$i;?><option value="<?php echo ($type_list["VALUE"]); ?>"><?php echo ($type_list["NAME"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
					</select>
				</div>
			</div>
