	...
	<form name="" action="" method="get">
									...
									<div class="table-search">
										<div class="form-group">
											<span class="input-group-btn search-btn">
												...
												<button event-name="delete" table-name="list" url="<?php echo U('Shopsetting/deleteTel');?>" message="确认要删选中的 x 条服务电话？|删除后信息不可恢复！" type="button" class="btn btn-sm btn-default">
													删除
		...
		<td colspan="100" style="text-align:center;">暂无数据！</td>
											...
											<?php if(is_array($page["list"])): $k = 0; $__LIST__ = $page["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($k % 2 );++$k;?><tr tag-name="level-1" tag-index="<?php echo ($k); ?>">
												...
												<?php if(is_array($list["tel_list"])): $i = 0; $__LIST__ = $list["tel_list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tel_list): $mod = ($i % 2 );++$i;?><tr tag-name="level-2" tag-index="level-2-<?php echo ($k); ?>" class="hide">
														...
														<td>
															<div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
																<a class="green" event-name="addEditTel" url="<?php echo U('Shopsetting/telInfo');?>" params="id=<?php echo ($tel_list["id"]); ?>"  dialog-title="服务电话编辑" submit-title="保存" flag="edit" href="javascript:void(0);" title="编辑">
																	<i class="icon-pencil bigger-130"></i>
																</a>
		
																<a class="red" event-name="delete" id-value="<?php echo ($tel_list["id"]); ?>" url="<?php echo U('Shopsetting/deleteTel');?>" message="确认要删除此服务电话？|删除后信息不可恢复！" href="javascript:void(0);" title="删除">
...
<div dialog-id="addEdit" class="hide">
	<form class="form-horizontal" role="form" form-id="addEdit" action="<?php echo U('Shopsetting/addEditTel');?>" method="post">
