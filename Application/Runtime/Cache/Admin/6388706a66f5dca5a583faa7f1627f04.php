							...
							<div class="form-group">
								...
								<div class="col-sm-4">
									<select event-name="getChildServer" url="<?php echo U('Inner/getServerTow');?>" target-id="tow-server" tag-id="room_type" name="service_id" class="form-control" style="width:290%;">
										<option value="0" message-type="0">请选择</option>
										<?php if(is_array($server)): $i = 0; $__LIST__ = $server;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$server_list): $mod = ($i % 2 );++$i;?><option value="<?php echo ($server_list["id"]); ?>" message-type="<?php echo ($server_list["service_type"]); ?>"><?php echo ($server_list["service_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
									</select>
								</div>
							</div>
