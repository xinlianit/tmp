...
class GoodsController extends CommonController {
    ...
    public function goodsMatch(){        
        ...
        $search['goods_name']     = I('get.goods_name','');        
        $this->assign($search);
        ...
        //商品名【如商家或商品太多（10W）时要优化此方法】
        if($search['goods_name']){
            //1.通过商品名获取商品ID
            $_map['goods_name'] = array('like','%'.$search['goods_name'].'%');
            $goods_list = D('GoodsInfo')->where($_map)->select();
            $goods_id = array();
            ...
            $hotel_id = array();
            if(!empty($goods_id)){
                $_map['goods_id'] = array('in',$goods_id);
                $match_list = M('GoodsMatch')->where($_map)->select();
                foreach ($match_list as $v) {
                    $hotel_id[] = $v['hotel_id'];
                }
            }
            $hotel_id = array_unique($hotel_id);
            unset($_map);
            //3.在商家表中对应商家ID的商家（最终条件）
            if(!empty($hotel_id)){
                $_where['id'] = array('in',$hotel_id);
            }
        }
    ...
    public function addMatch(){
        ...
        $model = M('goods_match');
        //检测同一酒店下是否有相同产品
        $_map['hotel_id'] = $_data['hotel_id'];
        $_map['goods_id'] = $_data['goods_id'];
        $_map['data_status'] = array('neq',99);
        if($model->where($_map)->count()){
            $this->ajaxReturnErr('商品重复');
        }
    ...
    public function addMatchs(){
        ...
        $templet_md5 = I('post.templet_md5');
        ...
        $res_info = D('Resource')->getResourceByMd5($templet_md5);
        //没有找到对应资源
        if(!$res_info){
            $this->ajaxReturnErr();
        }
        ...
        //资源地址
        $fullname = C('FASTDFS_URL').$res_info['res_path'];     
        $res = file_get_contents($fullname);        
        if ($res !== false) {
                $_tmp_name = TEMP_PATH.substr($res_info['res_path'],strrpos($res_info['res_path'],'/')+1);
                file_put_contents($_tmp_name, $res);
        } else {
            $this->ajaxReturnErr();
        }
        //获取文件内容
        $rs_excel = getExcel($_tmp_name);
        //删除临时文件
        unlink($_tmp_name);
        if(empty($rs_excel) || $rs_excel === false){
            return false;
        }
        
        $model_match = M('goods_match');
        $model_hotel = M('hotel_info');//商户模型
        $model_goods = M('goods_info');//商品模型
        $success = $failed = 0;

        foreach ($rs_excel as $val) {
            if(empty($val[0]) && is_null($val[0])){
                continue;
            }
            //1.商户是否存在
            unset($_map);
            $_map['id'] = $val[0];
            $_map['hotel_name'] = $val[1];
            $_map['data_status'] = array('neq',99);
            $_is_hotel = $model_hotel->where($_map)->count();
            if(!$_is_hotel){                
                $failed++;
                continue;
            }            
            ...
            unset($_map);
            $_map['id'] = $val[2];
            $_map['goods_name'] = $val[3];
            $_map['data_status'] = array('neq',99);
            $_is_goods = $model_goods->where($_map)->count();
            if(!$_is_goods){                
                $failed++;
                continue;
            }  
            ...
            $_map['goods_id'] = $val[2];
            $_map['data_status'] = array('neq',99);
            if($model_match->where($_map)->count()){    
                $failed++;
                continue;
            }
            //4.匹配入库
            $_data['hotel_id'] = $val[0];
            $_data['goods_id'] = $val[2];
            $_data['match_code'] = $val[4];
            $rs = $model_match->add($_data);
            if($rs === false){
                $failed++;
                continue;
            }
        ...
        if($success>0 && $failed>0){
            $this->ajaxReturnErr('部分成功，成功：'.$success.',失败：'.$failed);
        }
     ...
     * 获取批量配对模板
     */
    public function getMatchsTemp(){
        $data = array(
            array('商户ID','商户名称','商品ID','商品名称','配对号')
        );

        setExcelTemp($data,'match_templet');
    }
}