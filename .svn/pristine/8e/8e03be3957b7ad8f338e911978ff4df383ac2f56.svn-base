<?php
/**
 * @description of FastDFS.class.php	分布式文件服务器操作类
 * @package Common\Lib\FastDFS;
 * @author zhaoxz
 * @createtime 2016年11月3日 下午6:17:27
 */

namespace Common\Lib\FastDFS;
...
class FastDFS
{
    ...
    public function open($group_name = '')
    {
        if (self::$_client === null) {
            if (extension_loaded('fastdfs_client')) {
            	...
            	/**
            	 * @todo 未安装FDFS客户端扩展,程序实现
            	 */
            	self::$_client = new \Common\Lib\FastDFS\Client\FastDFSClient();
            }
     ...
     */
    public function fileExists($groupname = null, $filename = null)
    {
        if(!$groupname || !$filename)
            return false;
        $res = self::$_client->storage_file_exist($groupname, $filename, $this->tracker, $this->storage);
        if($res) {
            return $res;
        }
        return false;
    ...
    {
        if(!$groupname or !$filename)
        {
            return false;
        }
        $res = self::$_client->storage_delete_file($groupname, $filename, $this->tracker, $this->storage);
        if($res) {
            return $res;
        } 
        return false;
